 ...
 */
function assertInstallerClientConfigDirAlignment(): void {
  ...
  throw new Error(
    `[omc] CodeBuddy session detected, but the installer config dir is frozen to "${CLAUDE_CONFIG_DIR}" while the session resolves to "${resolved}". `
...
export function install(options: InstallOptions = {}): InstallResult {
  ...
  try {
    ...
    if (!projectScoped) {
      ...
      for (const backupPath of transaction.backups) log(`Backed up existing ${memoryFileName} to ${backupPath}`);
