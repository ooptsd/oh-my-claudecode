...
`;
  ...
  function installCapturingCoordinator(fixture: ReturnType<typeof createPluginFixture>, capturePath: string): void {
    const sourceSha256 = createHash('sha256').update(CANONICAL).digest('hex');
    writeFileSync(
      join(fixture.pluginRoot, 'bridge', 'claude-md-coordinator.cjs'),
      `const fs = require('node:fs');
...
`,
  ...
  it('sends CODEBUDDY.md/CODEBUDDY-omc.md in the coordinator REQUEST for a CodeBuddy session', () => {
    ...
    expect(result.status).toBe(0);
    const request = JSON.parse(readFileSync(capturePath, 'utf-8')) as Record<string, unknown>;
